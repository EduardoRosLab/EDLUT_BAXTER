...
void LIFTimeDrivenModel_1_1::LoadNeuronModel(string ConfigFile) throw (EDLUTFileException){
	FILE *fh;
	long Currentline = 0L;
...
LIFTimeDrivenModel_1_1::LIFTimeDrivenModel_1_1(string NeuronTypeID, string NeuronModelID): TimeDrivenNeuronModel(NeuronTypeID, NeuronModelID, SecondScale), eexc(0), erest(0), vthr(0), cm(0), texc(0),
		tref(0), grest(0){
}

LIFTimeDrivenModel_1_1::~LIFTimeDrivenModel_1_1(void)
{
}

void LIFTimeDrivenModel_1_1::LoadNeuronModel() throw (EDLUTFileException){
	this->LoadNeuronModel(this->GetModelID()+".cfg");
}

VectorNeuronState * LIFTimeDrivenModel_1_1::InitializeState(){
	return this->GetVectorNeuronState();
}


InternalSpike * LIFTimeDrivenModel_1_1::ProcessInputSpike(Interconnection * inter, double time){
...
bool LIFTimeDrivenModel_1_1::UpdateState(int index, double CurrentTime){
	//	//NeuronState[0] --> vm
	//	//NeuronState[1] --> gexc
...
enum NeuronModelOutputActivityType LIFTimeDrivenModel_1_1::GetModelOutputActivityType(){
	return OUTPUT_SPIKE;
}

enum NeuronModelInputActivityType LIFTimeDrivenModel_1_1::GetModelInputActivityType(){
	return INPUT_SPIKE;
}


ostream & LIFTimeDrivenModel_1_1::PrintInfo(ostream & out){
...
void LIFTimeDrivenModel_1_1::InitializeStates(int N_neurons, int OpenMPQueueIndex){
...
void LIFTimeDrivenModel_1_1::EvaluateSpikeCondition(float previous_V, float * NeuronState, int index, float elapsedTimeInNeuronModelScale){
...
void LIFTimeDrivenModel_1_1::EvaluateDifferentialEquation(float * NeuronState, float * AuxNeuronState, int index, float elapsed_time){
...
void LIFTimeDrivenModel_1_1::EvaluateTimeDependentEquation(float * NeuronState, int index, int elapsed_time_index){
	float limit=1e-9;
...
void LIFTimeDrivenModel_1_1::Calculate_conductance_exp_values(int index, float elapsed_time){
	//excitatory synapse.
	Set_conductance_exp_values(index, 0, expf(-elapsed_time*this->inv_texc));
}


bool LIFTimeDrivenModel_1_1::CheckSynapseType(Interconnection * connection){
...
void LIFTimeDrivenModel_1_1::loadIntegrationMethod(string fileName, FILE *fh, long * Currentline)throw (EDLUTFileException){
	...
	if (fscanf(fh, "%s", ident_type) == 1){
		...
		if (strncmp(ident_type, "Euler", 5) == 0){
			integrationMethod = (Euler<LIFTimeDrivenModel_1_1> *) new Euler<LIFTimeDrivenModel_1_1>(this);
		}
		else if (strncmp(ident_type, "RK2", 3) == 0){
			integrationMethod = (RK2<LIFTimeDrivenModel_1_1> *) new RK2<LIFTimeDrivenModel_1_1>(this);
		}
		else if (strncmp(ident_type, "RK4", 3) == 0){
			integrationMethod = (RK4<LIFTimeDrivenModel_1_1> *) new RK4<LIFTimeDrivenModel_1_1>(this);
		}
		else if (strncmp(ident_type, "BDF", 3) == 0 && atoi(&ident_type[3])>0 && atoi(&ident_type[3])<7){
			integrationMethod = (BDFn<LIFTimeDrivenModel_1_1> *) new BDFn<LIFTimeDrivenModel_1_1>(this, atoi(&ident_type[3]));
		}
		else if (strncmp(ident_type, "Bifixed_Euler", 13) == 0){
			integrationMethod = (Bifixed_Euler<LIFTimeDrivenModel_1_1> *) new Bifixed_Euler<LIFTimeDrivenModel_1_1>(this);
		}
		else if (strncmp(ident_type, "Bifixed_RK2", 11) == 0){
			integrationMethod = (Bifixed_RK2<LIFTimeDrivenModel_1_1> *) new Bifixed_RK2<LIFTimeDrivenModel_1_1>(this);
		}
		else if (strncmp(ident_type, "Bifixed_RK4", 11) == 0){
			integrationMethod = (Bifixed_RK4<LIFTimeDrivenModel_1_1> *) new Bifixed_RK4<LIFTimeDrivenModel_1_1>(this);
		}
		else if (strncmp(ident_type, "Bifixed_BDF", 11) == 0 && atoi(&ident_type[11]) == 2){
			integrationMethod = (Bifixed_BDFn<LIFTimeDrivenModel_1_1> *) new Bifixed_BDFn<LIFTimeDrivenModel_1_1>(this, atoi(&ident_type[11]));
		}
		else{
