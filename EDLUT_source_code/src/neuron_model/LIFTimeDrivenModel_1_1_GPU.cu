...
#include "../../include/neuron_model/LIFTimeDrivenModel_1_1_GPU2.h"
...
LIFTimeDrivenModel_1_1_GPU::LIFTimeDrivenModel_1_1_GPU(string NeuronTypeID, string NeuronModelID): TimeDrivenNeuronModel_GPU(NeuronTypeID, NeuronModelID, SecondScale), eexc(0), erest(0), vthr(0), cm(0), texc(0),
		tref(0), grest(0){
}

LIFTimeDrivenModel_1_1_GPU::~LIFTimeDrivenModel_1_1_GPU(void){
	DeleteClassGPU2();
}

void LIFTimeDrivenModel_1_1_GPU::LoadNeuronModel() throw (EDLUTFileException){
	this->LoadNeuronModel(this->GetModelID()+".cfg");
}

VectorNeuronState * LIFTimeDrivenModel_1_1_GPU::InitializeState(){
	return this->GetVectorNeuronState();
}


InternalSpike * LIFTimeDrivenModel_1_1_GPU::ProcessInputSpike(Interconnection * inter, double time){
...
__global__ void LIFTimeDrivenModel_1_1_GPU_UpdateState(LIFTimeDrivenModel_1_1_GPU2 ** NeuronModel_GPU2, double CurrentTime){
	(*NeuronModel_GPU2)->UpdateState(CurrentTime);
}


bool LIFTimeDrivenModel_1_1_GPU::UpdateState(int index, double CurrentTime){
	VectorNeuronState_GPU *state = (VectorNeuronState_GPU *) State;

	//----------------------------------------------
	if(prop.canMapHostMemory){
		LIFTimeDrivenModel_1_1_GPU_UpdateState<<<N_block,N_thread>>>(NeuronModel_GPU2, CurrentTime);
	}else{
...
enum NeuronModelOutputActivityType LIFTimeDrivenModel_1_1_GPU::GetModelOutputActivityType(){
	return OUTPUT_SPIKE;
}

enum NeuronModelInputActivityType LIFTimeDrivenModel_1_1_GPU::GetModelInputActivityType(){
	return INPUT_SPIKE;
}


ostream & LIFTimeDrivenModel_1_1_GPU::PrintInfo(ostream & out){
...
void LIFTimeDrivenModel_1_1_GPU::InitializeStates(int N_neurons, int OpenMPQueueIndex){
	...
	float initialization[] = {erest,0.0};
...
__global__ void LIFTimeDrivenModel_1_1_GPU_InitializeClassGPU2(LIFTimeDrivenModel_1_1_GPU2 ** NeuronModel_GPU2,
		float eexc, float erest, float vthr, float cm, float texc, float tref, float grest,
		char const* integrationName, int N_neurons, void ** Buffer_GPU){
	if(blockIdx.x==0 && threadIdx.x==0){
		(*NeuronModel_GPU2)=new LIFTimeDrivenModel_1_1_GPU2(eexc, erest,
			vthr, cm, texc, tref, grest, integrationName, N_neurons, Buffer_GPU);
	}
}

void LIFTimeDrivenModel_1_1_GPU::InitializeClassGPU2(int N_neurons){
	cudaMalloc(&NeuronModel_GPU2, sizeof(LIFTimeDrivenModel_1_1_GPU2 **));
	...
	LIFTimeDrivenModel_1_1_GPU_InitializeClassGPU2<<<1,1>>>(NeuronModel_GPU2,eexc, erest, vthr,
		cm, texc, tref, grest, integrationNameGPU, N_neurons, integrationMethod_GPU->Buffer_GPU);
...
__global__ void initializeVectorNeuronState_GPU2(LIFTimeDrivenModel_1_1_GPU2 ** NeuronModel_GPU2, int NumberOfVariables, float * InitialStateGPU, float * AuxStateGPU, float * StateGPU, double * LastUpdateGPU, double * LastSpikeTimeGPU, bool * InternalSpikeGPU, int SizeStates){
	if(blockIdx.x==0 && threadIdx.x==0){
...
void LIFTimeDrivenModel_1_1_GPU::InitializeVectorNeuronState_GPU2(){
	VectorNeuronState_GPU *state = (VectorNeuronState_GPU *) State;
...
__global__ void DeleteClass_GPU2(LIFTimeDrivenModel_1_1_GPU2 ** NeuronModel_GPU2){
	if(blockIdx.x==0 && threadIdx.x==0){
...
void LIFTimeDrivenModel_1_1_GPU::DeleteClassGPU2(){
    DeleteClass_GPU2<<<1,1>>>(NeuronModel_GPU2);
    cudaFree(NeuronModel_GPU2);
}


bool LIFTimeDrivenModel_1_1_GPU::CheckSynapseType(Interconnection * connection){
