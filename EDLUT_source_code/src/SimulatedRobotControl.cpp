...
#define REAL_TIME_NEURAL_SIM 0 // EDLUT's simulation mode (0=No real-time neural network simulation, 1=real-time simulation, 2=strict real-time simulation)
...
//    _CrtMemCheckpoint(&state0);
					int real_time_neural_simulation=0;
					if(REAL_TIME_NEURAL_SIM>0 && REAL_TIME_NEURAL_SIM<3){
						#ifdef _OPENMP 
							omp_set_nested(true);
							real_time_neural_simulation=REAL_TIME_NEURAL_SIM;
							if(REAL_TIME_NEURAL_SIM==1){
								cout<<"\nFixed REAL TIME SIMULATION option\n"<<endl;
							}
							if(REAL_TIME_NEURAL_SIM==2){
								cout<<"\nFixed STRICT REAL TIME SIMULATION option\n"<<endl;
							}
...
//     _CrtMemDumpAllObjectsSince(&state0);
					...
					cout << "Total Propagated Spikes and Events: " << neural_sim->GetTotalPropagateCounter()<<", "<< neural_sim->GetTotalPropagateEventCounter()<<endl;
