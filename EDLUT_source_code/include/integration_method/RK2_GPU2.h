 ...
 * \date May 2013
 *
 * This file declares a class which implement a fixed step second order Runge-Kutta integration method in GPU (this class is stored
...
class RK2_GPU2 : public FixedStep_GPU2 {
	public:
		...
		*
		* It generates a new second order Runge-Kutta Integration Method objectin GPU memory.
		*
		* \param TimeDrivenNeuronModel pointer to the time driven neuron model
		* \param Buffer_GPU integration method parameters
		*/
		__device__ RK2_GPU2(Neuron_Model_GPU2* NewModel, void ** Buffer_GPU) :FixedStep_GPU2(Buffer_GPU, NewModel->timeScale), neuron_model(NewModel){
		...
		__device__ void NextDifferentialEquationValues(int index, int SizeStates, float * NeuronState){
			...
			neuron_model->EvaluateTimeDependentEquation(offset2, offset1, NeuronState, this->elapsedTimeInNeuronModelScale, 0);
			neuron_model->EvaluateDifferentialEquation(offset2, offset1, NeuronState, AuxNeuronState2, this->elapsedTimeInNeuronModelScale);
