...
class Bifixed_RK2 : public BiFixedStep {
	public:
		...
		*/
		 ...
		 */
		Bifixed_RK2(Neuron_Model * NewModel) :BiFixedStep("Bifixed_RK2"), neuron_model(NewModel){
		}
		...
		void NextDifferentialEquationValues(int index, float * NeuronState){
			...
			if (integrationMethodState[index]>0){
				for (int iteration = 0; iteration<ratioLargerSmallerSteps; iteration++){
					...
					//2nd term
					for (int j = 0; j<this->neuron_model->N_DifferentialNeuronState; j++){
						NeuronState[j] += AuxNeuronState1[j] * bifixedElapsedTimeInNeuronModelScale;
					}


					this->neuron_model->EvaluateTimeDependentEquation(NeuronState, index, 1);
					this->neuron_model->EvaluateDifferentialEquation(NeuronState, AuxNeuronState2, index, bifixedElapsedTimeInNeuronModelScale);


					for (int j = 0; j<this->neuron_model->N_DifferentialNeuronState; j++){
						NeuronState[j] += (-AuxNeuronState1[j] + AuxNeuronState2[j])*bifixedElapsedTimeInNeuronModelScale*0.5f;
					}
