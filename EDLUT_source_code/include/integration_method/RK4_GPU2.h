 ...
 * \date May 2013
 *
 * This file declares a class which implement a fixed step fourth order Runge-Kutta integration method in GPU (this class is stored
...
class RK4_GPU2 : public FixedStep_GPU2 {
	public:
		...
		*/
		__device__ RK4_GPU2(Neuron_Model_GPU2* NewModel, void ** Buffer_GPU) :FixedStep_GPU2(Buffer_GPU, NewModel->timeScale), neuron_model(NewModel){
		...
		__device__ void NextDifferentialEquationValues(int index, int SizeStates, float * NeuronState){
			...
			//1st term
			neuron_model->EvaluateDifferentialEquation(index, SizeStates, NeuronState, AuxNeuronState1, this->elapsedTimeInNeuronModelScale);
			
			//2nd term
			for (int j=0; j<this->neuron_model->N_DifferentialNeuronState; j++){
				AuxNeuronState[j*offset1 + offset2]= NeuronState[j*SizeStates + index] + AuxNeuronState1[j*offset1 + offset2]*this->elapsedTimeInNeuronModelScale*0.5f;
			}
			...
			neuron_model->EvaluateTimeDependentEquation(offset2, offset1, AuxNeuronState, this->elapsedTimeInNeuronModelScale*0.5f, 0);
			neuron_model->EvaluateDifferentialEquation(offset2, offset1, AuxNeuronState, AuxNeuronState2, this->elapsedTimeInNeuronModelScale);

			//3rd term
			for (int j=0; j<this->neuron_model->N_DifferentialNeuronState; j++){
				AuxNeuronState[j*offset1 + offset2]= NeuronState[j*SizeStates + index] + AuxNeuronState2[j*offset1 + offset2]*this->elapsedTimeInNeuronModelScale*0.5f;
			}

			neuron_model->EvaluateDifferentialEquation(offset2, offset1, AuxNeuronState, AuxNeuronState3, this->elapsedTimeInNeuronModelScale);

			//4rd term
			for (int j=0; j<this->neuron_model->N_DifferentialNeuronState; j++){
				AuxNeuronState[j*offset1 + offset2]= NeuronState[j*SizeStates + index] + AuxNeuronState3[j*offset1 + offset2]*this->elapsedTimeInNeuronModelScale;
			}

			neuron_model->EvaluateTimeDependentEquation(offset2, offset1, AuxNeuronState, this->elapsedTimeInNeuronModelScale*0.5f, 0);
			neuron_model->EvaluateDifferentialEquation(offset2, offset1, AuxNeuronState, AuxNeuronState4, this->elapsedTimeInNeuronModelScale);


			for (int j=0; j<this->neuron_model->N_DifferentialNeuronState; j++){
				NeuronState[j*SizeStates + index]+=(AuxNeuronState1[j*offset1 + offset2]+2*(AuxNeuronState2[j*offset1 + offset2]+AuxNeuronState3[j*offset1 + offset2])+AuxNeuronState4[j*offset1 + offset2])*this->elapsedTimeInNeuronModelScale*0.166666666667f;
			}

			//Finaly, we evaluate the neural state variables with time dependence.
			//neuron_model->EvaluateTimeDependentEquation(index, SizeStates, NeuronState, this->elapsedTimeInNeuronModelScale, 1);
			for (int j=this->neuron_model->N_DifferentialNeuronState; j<this->neuron_model->N_NeuronStateVariables; j++){
