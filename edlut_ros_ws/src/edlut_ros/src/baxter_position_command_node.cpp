/***************************************************************************
 *                           baxter_position_command_node.cpp              *
 *                           -------------------                           *
...
// This node transforms a given input position command into an output
...
public:
		...
		disable_gravity_compensation(disable_gravity_compensation)
	{
		...
		this->position_values = std::vector<double> (this->joint_list.size(),0.0);

		this->in_learning = false;

		this->output_Publisher = nh.advertise<baxter_core_msgs::JointCommand>(output_topic, 1);
		ROS_INFO("Baxter Position Command: Writing position commands to topic %s", output_topic.c_str());
	...
	void EventCallback(const edlut_ros::AnalogCompact::ConstPtr& trq){
		// ROS_DEBUG("Baxter position Command Node: Received event S0");
		...
		for (unsigned int i=0; i<trq->names.size(); ++i){
			...
			if (index!=-1) {
				this->position_values[index] = trq->data[i];
			}
		}
		return;
	}
	...
	void ControlEventCallback(const edlut_ros::LearningState::ConstPtr& lear){
		// ROS_DEBUG("Baxter position Command Node: Received control event");
	...
	void PublishCommand(){
		if (this->in_learning){
			...
			newMsg.command = this->position_values;
...
{
	// Set up ROS.
	ros::init(argc, argv, "baxter_position_command", ros::init_options::NoSigintHandler);
	...
	ROS_INFO("Baxter position Command node initialized: reading from topic %s and writing to topic %s with sampling frequency %f", input_topic.c_str(), output_topic.c_str(), sampling_frequency);
	...
	ROS_INFO("Ending Baxter position Command node");
